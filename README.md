...
把「想法 → PRD → 架構 ∥ UI 設計 → 使用者故事 → 交付 → 自動實作」串成 pipeline。每個 stage / agent / delivery target 從第一天起都是可獨立打包安裝的 plugin。

> **狀態**：需求工程主線已端到端打通（PRD → 架構 ∥ UI 設計 → Stories → 交付 → 自動實作 batch），里程碑 M1–M5 皆已落地，並有專案級 **Flight Log** 總覽。
...
- **Stage 生成**（`prd` / `architecture` / `ui_design` / `stories`）：sync harness fix-loop（生成 + validator + 對話補充），核准後解鎖下游 stage。
- **UI 設計**（與架構平行）：UI Designer agent 依 PRD 產出設計理念、design tokens 與每畫面自包含 HTML 原型；前端 sandboxed iframe 直接預覽、可切換畫面與 document / preview / code view；設計稿（去 HTML）餵給 stories 對齊畫面拆解。
