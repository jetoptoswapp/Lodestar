...
from plugin_api import ModelAdapter, rate_limit

_log = logging.getLogger("plugins.claude_cli")
...
def _extract_error(stdout: str) -> str:
    """從 stream-json stdout 抽出可讀的錯誤原因。

    claude `-p` 的錯誤幾乎都在 stdout（result 事件 is_error / 非 success subtype、system error /
    api_retry 事件），stderr 多為空 → 只看 stderr 會得到無資訊的「exited 1: 」。這裡掃 stdout 把真正
    原因撈出來，讓失敗可診斷（而非靜默「沒反應」）。"""
    reasons: list[str] = []
    for line in (stdout or "").splitlines():
        ...
        if t == "result" and (ev.get("is_error") or ev.get("subtype") not in (None, "success")):
            reasons.append(str(ev.get("result") or ev.get("error")
                                or ev.get("subtype") or "result error"))
        elif t == "error":
            reasons.append(str(ev.get("error") or ev.get("message") or s[:200]))
        elif t == "system" and ev.get("subtype") == "api_retry":
            reasons.append(f"api_retry status={ev.get('error_status')} {ev.get('error')}")
    # 去重保序
    seen, out = set(), []
    for r in reasons:
...
def _sleep(seconds: float) -> None:
    """等待用量上限重置（sync，跑在 worker thread 故不凍 event loop）。抽成 module 函式便於測試覆寫。"""
...
def _invoke(prompt: str, *, allowed_tools: tuple[str, ...] = (), workspace_dir: str = "") -> str:
    ...
    用量上限（5hr 訂閱方案）：跑完先看 stdout+stderr 是否撞上限 → 是則算解鎖時間、睡到那刻、重跑同
    指令自動續跑（封頂 cfg.max_cycles 輪）。harness stage 生成跑在 worker thread，sync sleep 不凍 event loop。"""
    ...
    for cycle in range(cfg.max_cycles + 1):
        ...
        # 不論 exit code，先判是否撞用量上限（訊息可能在 stdout 的 result/error 事件或 stderr）
        combined = f"{proc.stdout or ''}\n{proc.stderr or ''}"
        signal = rate_limit.detect(combined, now=datetime.now())
        if signal is not None:
            if cycle < cfg.max_cycles:
                wait_s = rate_limit.seconds_until(signal, now=datetime.now(), cfg=cfg)
                _log.warning("claude-cli 撞用量上限：%s；約 %s 後自動續跑",
                             signal.message, rate_limit.humanize(wait_s))
                _sleep(wait_s)
                continue
            # 續跑額度用盡仍受限 → 明確以「用量上限」收尾（而非泛用 exit 訊息）
            raise RuntimeError(
                f"claude-cli 仍受用量上限限制（已續跑 {cfg.max_cycles} 輪）：{signal.message}")

        if proc.returncode != 0:
            # 真正原因多在 stdout（stream-json result/error 事件），stderr 常為空 → 先撈 stdout 再退回 stderr
            detail = _extract_error(proc.stdout) or (proc.stderr or "").strip()[:500] or "(no error detail)"
            raise RuntimeError(f"claude-cli exited {proc.returncode}: {detail}")
        return _parse_stream_json(proc.stdout)

    # for 迴圈每一輪都 return / continue / raise，到不了此處；防禦性收尾。
    raise RuntimeError("claude-cli 未產生結果")
